...
const CategoryManagementAdd: React.FC = () => {
  ...
  // Auto-generate code from category name
  useEffect(() => {
    if (formData.categoryName && !formData.code) {
      const baseCode = formData.categoryName
        .toUpperCase()
        .replace(/[^A-Z0-9]/g, '')
        .substring(0, 8);
      
      const timestamp = Date.now().toString().slice(-4);
      const generatedCode = `CAT${baseCode}${timestamp}`.substring(0, 12);
      
      setFormData(prev => ({ ...prev, code: generatedCode }));
    }
  }, [formData.categoryName, formData.code]);
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => navigate('/category-management-list')}
                ...
                >
                  <div className="space-y-4">
                    ...
                    <FormGroup
                      ...
                      required
                      info="Code will be auto-generated from category name. Use only uppercase letters and numbers."
                      darkMode={darkMode}
                    />
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <p className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                          {formData.categoryName || 'New Category'}
                        </p>
                      </div>
                      <div>
                        <p className={`text-sm font-medium ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
